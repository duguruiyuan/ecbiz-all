...
public class XmlUtility {
	...
	/**
	 * transformer tamplates cache
	 */
	private static HashMap<String, Object> transformerTemplates = new HashMap<String, Object>();
	...
	public static Document createDocument(String xmlString) throws Exception {
		/*InputStream xmlIS = new ByteArrayInputStream(xmlString.getBytes("UTF-8"));
		return(createDocument(xmlIS));*/
	...
	public static Document createDocument(InputStream xmlInputStream) throws Exception {
		...
		try {
			...
			doc = fact.newDocumentBuilder().parse(xmlInputStream);
	...
	public static Document createDocument(String docTypeName, String systemID) throws Exception {
		try {
			...
			if (isValidation && docTypeName != null && systemID != null) {
				DocumentType t = doc.createDocumentType(docTypeName, null, systemID);
				doc.appendChild(t);
			}*/
	...
	public static Document createDocument() throws Exception {
		try {
			...
			//doc = fact.newDocumentBuilder().getDOMImplementation().createDocument(null, null, null);	// DOM Level 2
			return(doc);
		} catch (Exception e) {
			logger.error("error", e);
			throw e;
	...
	public static String serializeDocument(Document doc) throws Exception {
		try {
			...
			// Apache xerces 1.3
			org.apache.xml.serialize.XMLSerializer ser = new org.apache.xml.serialize.XMLSerializer(xmlOS, new org.apache.xml.serialize.OutputFormat(doc, "UTF-8", isXmlOutputIndenting));
			ser.serialize(doc);
	 ...
	 * @param values is the new values of text of the element
	 */
	public static void setElementValue(Element elm, List<?> values) throws Exception {
	 ...
	 * @param values is the values of text of the element
	 */
	public static void setElementValue(Node contextNode, String elmName, List<?> values) throws Exception {
		Element elm = (Element)selectSingleNode(contextNode, elmName);
		if (elm == null) elm = (Element)createChildPath(contextNode, elmName);
		setElementValue(elm, values);
	}
	 ...
	 */
	public static List<?> getValues(Node contextNode, String elmPath) {
		if (contextNode == null || elmPath == null) return(null);
	...
	public static String transformToString(String xmlString, String xslSystemID) throws Exception {
		try {
			...
			transformToStream(xmlString, xslSystemID, outputStream);
	...
	public static String transformToString(InputStream xmlStream, String xslSystemID) throws Exception {
		try {
			// JAXP, Apache xalan 2.0.1 and xerces 1.3 (note: xalan 2.0.0 has some bugs)
			/*ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
			transformToStream(xmlStream, xslSystemID, outputStream);
			return(outputStream.toString("UTF-8"));*/
	...
	public static String transformToString(Reader xmlReader, String xslSystemID) throws Exception {
		try {
			// JAXP, Apache xalan 2.0.1 and xerces 1.3 (note: xalan 2.0.0 has some bugs)
			/*ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
			transformToStream(xmlReader, xslSystemID, outputStream);
			return(outputStream.toString("UTF-8"));*/
	...
	public static String transformToString(Node xmlNode, String xslSystemID) throws Exception {
		try {
			// JAXP, Apache xalan 2.0.1 and xerces 1.3 (note: xalan 2.0.0 has some bugs)
			/*ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
			transformToStream(xmlNode, xslSystemID, outputStream);
			return(outputStream.toString("UTF-8"));*/
	...
	private static void transformToResult(String xmlString, String xslSystemID, javax.xml.transform.Result result) throws Exception {
		//transformToResult(new ByteArrayInputStream(xmlString.getBytes("UTF-8")), xslSystemID, result);
	...
	private static javax.xml.transform.Transformer createTransformer(String xslSystemID) throws Exception {
		...
		if (xslSystemID == null) {	// get a transformer to copy source to result (DOM, stream, or for serialization)
			...
			//	logger.error("TransformerFactoryConfigurationError");
			//	throw new Exception("TransformerFactoryConfigurationError");
			//}
...
//						propValue = new String(PwdEncoder.decodePassword((String) propValue));
//				}
...
//							val = new String(PwdEncoder.decodePassword((String) val));
//					}
	...
	public static Map<String, Object> getPropertyParams(Node ctx) throws Exception {
		if (ctx == null)
			return (null);
		NodeList parmNodes = selectNodeList(ctx, "param");
		...
		for (int i = 0; i < parmNodes.getLength(); ++i) {
			...
			// String paramValue = parmNode.getAttribute("value");
			Node t = XmlUtility.selectSingleNode(parmNode, "text()");
			String parmValue = t.getNodeValue();
