...
public class LetterConvert {
	...
	private static LetterConvert chineseSpelling = new LetterConvert();

	public static LetterConvert getInstance() {
	...
	public String getFirSpelling(String src) {
		String firSpelling = "";

		String[] splitedSrc = src.split("");
		for (int i = 0; i < splitedSrc.length; i++) {
			if (splitedSrc[i] != "") {
				setResource(splitedSrc[i]);
				String spelling = getSpelling();
				if (spelling.length() > 1)
					firSpelling += spelling.substring(0, 1);
				else
					firSpelling += spelling;
			}
		}
	...
	public String getAllSpelling(String src) {
		String allSpelling = "";
	...
	/**
	 * get letter for linux 获取指定字符串的首字母
	 * 
	 * @param text
	 * @return
	 */
	public static String getValue(String text) {
		if (text == null || "".equals(text))
			return null;
		//算法缺陷，待改进
		if(text!=null&&text.endsWith("讴歌")){
			return "O";
		}
		LetterConvert finder = LetterConvert.getInstance();
		char[] chars = new String(text).toCharArray();
		//System.out.println(chars[0]);
		String letter = finder.getFirSpelling(chars[0] + "");
		return letter.toUpperCase();
		// return getValueForLinux(text);
	}

	/**
	 * get letter for linux
	 * 
	 * @param inputString
	 * @return
	 */
	public static String getValueForLinux(String inputString) {
	...
	public static String getFirstValue(String text) {
		...
		for (int i = 0; i < chars.length; i++) {
			sf.append(finder.getFirSpelling(chars[i] + ""));
		}
		String aString = sf.toString().toLowerCase().replaceAll(" ", "");
		return StringUtil.StringFilter(aString);
	}
	...
	public static String getValues(String text) {
		...
		char[] chars = new String(text).toCharArray();
		for (int i = 0; i < chars.length; i++) {
			sf.append(finder.getAllSpelling(chars[i] + ""));
		}
		String aString = sf.toString().toLowerCase().replaceAll(" ", "");
		return StringUtil.StringFilter(aString);
	}
	...
	public static void main(String[] args) {
		//System.out.println(getValue("开"));
		//System.out.println(getValue("开"));
		System.out.println(getValue("讴歌"));
		System.out.println(getValue("哦讴歌"));
		System.out.println(getValue("讴歌1"));
		System.out.println(getValue("哦讴歌1"));
		//System.out.println(getValue("哦讴歌"));
	}
}
