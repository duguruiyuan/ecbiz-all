 ...
 * @version $Revision: 1.1 $ $Date: 2009/05/08 05:50:27 $
 * @see BeanUtils
 * @since 1.7
 */
...
public class BeanUtilsBean {
     ...
     *
     * <p><strong>WARN</strong> - Indexed and mapped properties that do not
	...
	public void populate(Object bean, Map properties)
        ...
        while (names.hasNext()) {
            ...
            String addin = null;
            try {
            	String[] temp = (String[]) properties.get(name + "_pattern");
            ...
            setProperty(bean, name, value, addin);
        }
    ...
    @SuppressWarnings("unchecked")
	public void setProperty(Object bean, String name, Object value, String addin) throws IllegalAccessException, InvocationTargetException {
        // Trace logging (if enabled)
        if (log.isTraceEnabled()) {
        ...
        if (type.isArray() && (index < 0)) { // Scalar value into array
            if (value == null) {
                ...
                newValue = getConvertUtils().convert((String[]) values, type, addin);
            } else if (value instanceof String) {
                String values[] = new String[1];
                values[0] = (String) value;
                newValue = getConvertUtils().convert((String[]) values, type, addin);
            } else if (value instanceof String[]) {
                newValue = getConvertUtils().convert((String[]) value, type, addin);
            } else {
                newValue = value;
            }
        } else if (type.isArray()) {         // Indexed value into array
            if (value instanceof String) {
                newValue = getConvertUtils().convert((String) value, type.getComponentType(), addin);
            } else if (value instanceof String[]) {
                newValue = getConvertUtils().convert(((String[]) value)[0], type.getComponentType(), addin);
            } else {
                newValue = value;
            }
            ...
            if ((value instanceof String) || (value == null)) {
				...
				newValue = getConvertUtils().convert(((String[])value)[0], type, addin);
            } else if (getConvertUtils().lookup(value.getClass()) != null) {
                newValue = getConvertUtils().convert(value.toString(), type, addin);
            } else {
                newValue = value;
            }
        }
