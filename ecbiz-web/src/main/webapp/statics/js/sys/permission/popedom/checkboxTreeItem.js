...
WebFXCheckBoxTreeItem.prototype.toString = function (nItem, nItemCount) {
	...
	var label = "&nbsp;" + this.text.replace(/</g, '&lt;').replace(/>/g, '&gt;')+" "+this.addition;
	...
	var tempStr = "<input type=\"checkbox\"" + " class=\"tree-check-box\"" +
		...
		" value=\"" + this.value + "\" id=\"" + this.id + "-input\" name=\"moduleIds\" " + (this._disabled?" disabled ": "") + ">";
	str += tempStr;	
	// end insert checkbox
...
//检查子节点被选中的个数 add by kevin
function statusBoxCount(pnode){
	var node = pnode.childNodes;
	for(var i=0; i<node.length; i++){
		var checked = getCheckBox(node[i].id).checked;
